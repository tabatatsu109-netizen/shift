  ...
  stores.forEach(store=>{
    ["AM","PM"].forEach(sh=>{
      ...
      dates.forEach(d=>{
        ...
        tbody += `<td class="cell-day" data-date="${d.iso}" data-store="${store.id}" data-shift="${sh}">`;
        tbody += `<div class="chip-list">`;
        assigned.forEach(a=>{
          const st = getStaff(a.staffId);
          tbody += `<span class="cell-chip"><span class="chip-name" data-staffid="${a.staffId}">${st?escapeHtml(st.name):"?"}</span><button class="chip-remove" data-aid="${a.id}">×</button></span>`;
        });
        tbody += `</div>`;
...
document.getElementById("shiftTable").addEventListener("click", (e)=>{
  const nameEl = e.target.closest(".chip-name");
  if(nameEl){ openStaffModal(nameEl.dataset.staffid, shiftMonthInput.value); return; }
...
document.getElementById("staffStatsTable").addEventListener("click", (e)=>{
  const el = e.target.closest(".staff-name-link");
  if(el) openStaffModal(el.dataset.staffid, shiftMonthInput.value);
});

/* ---------- スタッフ統計の共通計算 ---------- */
function computeStaffStats(ym){
  const shift = ensureShift(ym);
  ...
  db.staff.forEach(st=>{
    ...
    const byStore = {};
    myAssign.forEach(a=>{ byStore[a.storeId] = (byStore[a.storeId]||0)+1; });
    result[st.id] = {
      shiftCount: myAssign.length,
      workDays: workDays.size,
      offDays: dates.length - workDays.size,
      maxStreak,
      byStore
    };
  });
...
/* ---------- スタッフ詳細モーダル ---------- */
function openStaffModal(staffId, ym){
  ...
  const targetYm = ym || shiftMonthInput.value || currentMonthStr();
  const stats = computeStaffStats(targetYm)[staffId] || {shiftCount:0,workDays:0,offDays:0,maxStreak:0,byStore:{}};
  const storeBreak = Object.entries(stats.byStore)
    .map(([sid,cnt])=>`${getStore(sid)?.name||"?"}：${cnt}コマ`).join("／") || "-";
  const offDayLabel = st.baseOffDays.map(d=>WEEKDAYS[d]).join("・") || "なし";

  body.innerHTML = `
    <h3>${escapeHtml(st.name)}</h3>
    <dl>
      <dt>資格</dt><dd>${escapeHtml(st.qualification||"-")}</dd>
      <dt>基本勤務店舗</dt><dd>${escapeHtml(getStore(st.baseStore)?.name||"-")}</dd>
      <dt>基本休み曜日</dt><dd>${escapeHtml(offDayLabel)}</dd>
      <dt>給与</dt><dd>${st.wage ? st.wage+"円 / "+(st.wageType==="hourly"?"時給":"日給") : "-"}</dd>
      <dt>${targetYm} の勤務状況</dt>
      <dd>勤務日数：${stats.workDays}日　／　休み：${stats.offDays}日　／　最大連勤：${stats.maxStreak}日</dd>
      <dd>店舗別：${escapeHtml(storeBreak)}</dd>
      <dt>メモ</dt>
  ...
  let rows = db.staff.map(st=>{
    ...
    return `<tr>
      ...
      <td class="row-actions">
        <button class="btn btn-xs btn-light" data-detail="${st.id}">詳細</button>
...
document.getElementById("staffTable").addEventListener("click", (e)=>{
  ...
  if(detailId){ openStaffModal(detailId, shiftMonthInput.value); return; }
...
function renderCalendar(){
  ...
  dates.forEach(d=>{
    const am = shift.assignments.filter(a=>a.date===d.iso && a.storeId===storeId && a.shift==="AM").map(a=>surnameOf(a.staffId));
    const pm = shift.assignments.filter(a=>a.date===d.iso && a.storeId===storeId && a.shift==="PM").map(a=>surnameOf(a.staffId));
...
function surnameOf(staffId){
  const st = getStaff(staffId);
  if(!st) return "?";
  return st.name.split(/[\s　]+/)[0];
}
